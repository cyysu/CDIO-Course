...
//在OLED GRAM指定地址写入一个数据
//WriteAddr  :写入数据的目的地址    
//DataToWrite:要写入的数据
void OLED_WriteOneByte(u8 WriteAddr,u8 DataToWrite)
{
	IIC_Start();
	IIC_Send_Byte(OLED_ADDRESS);		//发送OLED器件地址
	IIC_Wait_Ack();
	...
	delay_ms(5);					 //这个延时是必须的，保证下次发送之前，上次的数据已经被接收
}

//向OLED写命令
void OLED_WriteCmd(unsigned char I2C_Command)//写命令
{
	OLED_WriteOneByte(0x00, I2C_Command);
}

//向OLED写数据
void OLED_WriteDat(unsigned char I2C_Data)//写数据
{
	OLED_WriteOneByte(0x40, I2C_Data);
}
